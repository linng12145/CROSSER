...
def load_dataset(args, data_format):
    ...
    if data_format is 'csv':
        ...
        logger.info("train data size {}, test data size {}, cell tower num {}".format(len(lbs_train), len(lbs_test),
                                                                                len(id2loc)))
...
def get_masks_and_count_tokens_trg(trg_token_ids_batch, pad_token_id):
    ...
    trg_mask = (trg_token_ids_batch != pad_token_id).view(batch_size, -1)
    num_trg_tokens = torch.sum(trg_mask.long())
...
def evaluation(inputs, preds, truths, id2loc, maxlen):
    ...
    logger.info("average recall {}, average precision {}, average recovery {}, average micro-precision {}". \
