...
def load_dataset(args, data_format):
    ...
    if data_format is 'csv':
        ...
        logger.info("train data size {}, val data size {}, test data size {}, cell tower num {}"\
    ...
    logger.info("train num {}, val num {}, test num {}, target {}, " \
...
def get_masks_and_count_tokens_src(src_token_ids_batch, pad_token_id):
    ...
    src_mask = (src_token_ids_batch != pad_token_id).view(batch_size, -1)
...
def evaluation(preds, targets, lengths):
    ...
    logger.info("average precision {}, average recall {}, average f1 {}". \
          format(pre, rec, f1))
    return pre, rec, f1
...
def evaluation_multiclass(preds, targets, lengths):
    ...
    logger.info("precision for each class {}".format(pre_))
    logger.info("recall for each class {}".format(rec_))
    ...
    logger.info("average precision {:.4f}, weighted {:.4f};  average recall {:.4f}, weighted {:.4f}; average f1-micro {:.4f}, average f1-macro {:.4f}". \
