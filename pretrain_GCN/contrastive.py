...
def load_graph_data(dataset):
    ...
    edge_path = os.path.join(data_path, 'graph_A.csv')
    edge_df = pd.read_csv(edge_path)

    all_nodes = sorted(set(edge_df['src']).union(set(edge_df['dst'])))
    num_nodes = len(all_nodes)

    node_to_idx = {node: idx for idx, node in enumerate(all_nodes)}
    src_indices = [node_to_idx[src] for src in edge_df['src']]
    dst_indices = [node_to_idx[dst] for dst in edge_df['dst']]

    G = nx.DiGraph()
    edge_index = torch.tensor([src_indices, dst_indices], dtype=torch.long)
    src, dst, weights = edge_df['src'].values.tolist(), edge_df['dst'].values.tolist(), edge_df['weight'].values.tolist()
    G.add_weighted_edges_from(zip(src, dst, weights))
    adj_graph = nx.to_numpy_array(G)
    x = torch.tensor(adj_graph, dtype=torch.float)


    return Data(x=x, edge_index=edge_index, num_nodes=num_nodes)

def get_clustered_data(dataset, cache_dir, cross_link, node_feature_dim, cl_init_method='learnable', cross_link_ablation=False, dynamic_edge='none',dynamic_prune=0.0,split_method='RandomWalk'):


    pyg_dataset = [load_graph_data(data) for data in dataset]
    data_list = [preprocess(data, node_feature_dim) for data in pyg_dataset]
