...
import sys
sys.path.append('../')
from pretrain_GCN.gcn_conv import GCNConv
...
class GCN(torch.nn.Module):
    ...
    def __init__(self, num_features,
                       ...
                       res_branch="BNConvReLU", dropout=0, reconstruct=0.0,
                       edge_norm=True):
...
def get_model(num_features, hid_dim, num_conv_layers, dropout, reconstruct):
    return GCN(num_features=num_features, hidden=hid_dim, num_conv_layers=num_conv_layers, dropout=dropout, reconstruct = reconstruct, gfn=False)
